        ...
        try:
            for keyword in keyword_list:
                ...
                for link in result_links: 
                    try:  # Note: sometimes the driver raise an exception when it face the 'aw snap', so the try-except is necessary
                        business_details = scrape_business_details(driver=driver_headless, link=link) # todo: add explicit wait and send only driver
                    except Exception as e:   
                        logging.warning(f'exception was raised while scraping a link: {e}')
                        print(f'Aww Snap has occured! {keyword}')  

                        # attempt to scrape the business again but if it still raises exception, then end the city and move to next
